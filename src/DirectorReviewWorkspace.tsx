...
import { applyReviewAction, updateReview, type ReviewAction, type ReviewBeat, type ReviewState, type ReviewWorkspaceData } from './director-review.ts';
...
function DecisionCard({ beat, data, decision, selected, onSelect, onAction }: { beat: ReviewBeat; data: ReviewDataPayload; decision: ReviewState['decisions'][number]; selected: boolean; onSelect: () => void; onAction: (action: ReviewAction, options?: { operation?: EditOperation; displayText?: string; reason?: string }) => void }): React.ReactElement {
  ...
  return <article className={`decision-card ${selected ? 'selected' : ''} status-${decision.status}`} onClick={onSelect}>
    ...
    <div className="card-grid"><Metric label="AI recommendation" value={section.visualRecommendation ?? 'speaker-full'} /><Metric label="Retention" value={beat.noBroll ? 'NO B-ROLL' : 'SEARCH → SELECT'} tone={beat.noBroll ? 'muted' : 'gold'} /><Metric label="Confidence" value={`${Math.round(section.confidence * 100)}%`} /></div>
    {isBroll && beat.selectedAsset && <div className="asset-summary"><div className="asset-thumb">{data.assetPreviewUrls[beat.selectedAsset.id] && <img src={data.assetPreviewUrls[beat.selectedAsset.id]} alt="Pressure Cooker.png" />}</div><div><small>FINAL AI-RESOLVED VISUAL</small><strong>{beat.selectedAsset.fileName}</strong><span>{beat.brollDecision?.candidates.find((candidate) => candidate.assetId === beat.selectedAsset?.id)?.score.toFixed(3)} score · {beat.placement?.decision === 'place' ? beat.placement.selectedRegion : 'review'} placement</span></div></div>}
    ...
    {replaceMode && isBroll && <div className="replace-picker"><small>REPLACE FROM INDEXED, RIGHTS-SAFE MEDIA</small>{beat.candidates.filter((candidate) => candidate.eligible && candidate.asset).map((candidate) => <button key={candidate.assetId} onClick={() => { setReplaceMode(false); onAction('replace-broll', { operation: beat.originalOperation, reason: `Selected ${candidate.asset?.fileName} from the indexed eligible candidate list.` }); }}>Use {candidate.asset?.fileName}</button>)}{beat.candidates.filter((candidate) => candidate.eligible && candidate.asset).length === 0 && <span>No eligible local replacement is available.</span>}</div>}
    <CandidateInspector beat={beat} data={data} />
  </article>;
}

export const DirectorReviewWorkspace: React.FC<{ data: ReviewDataPayload }> = ({ data }) => {
  const playerRef = useRef<HTMLVideoElement>(null);
  const [review, setReview] = useState<ReviewState>(() => {
    try {
      const saved = window.localStorage.getItem(`director-review:${data.projectId}`);
      if (saved) return JSON.parse(saved) as ReviewState;
  ...
  return <div className="review-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">✦</span><div><strong>DIRECTOR REVIEW</strong><small>AI-assisted sermon editing</small></div></div><div className="project-title"><span className="live-dot" />{data.title}<span className="language">বাংলা · 16:9</span></div><div className="top-actions"><span className={`readiness ${resolved.readiness.ready ? 'ready' : 'blocked'}`}>{resolved.readiness.label}</span><button className="save" onClick={() => window.alert('Review state persisted in review/director-review.json')}>Save review</button></div></header>
